...
employerRoutes.route("/cv").get(function (req, res) {
...
// Create a new CV for an applicant.
employerRoutes.route("/cv/add/:id").post(function (req, response) {
  let db_connect = dbo.getDb("JobQuest");
  ...
  db_connect.collection("CVs").insertOne(myobj, function (err, res) {
    if (err) throw err;
    response.json(res);
  });
})
